# Topic 11 Chat Application

Фінальний приклад теми 11: real-time чат на `Express`, `Socket.IO`, `Prisma` і SQLite. Проєкт показує базову авторизацію через cookie-сесію, Socket.IO-кімнати і збереження повідомлень у базі.
...
З root папки `topic_11` встановити залежності:
...
Заповнити `DATABASE_URL` для SQLite:

```env
DATABASE_URL="file:./dev.db"
```

З root папки `topic_11` застосувати міграції і згенерувати клієнт:
...
Запустити сервер з root папки `topic_11`:
...
public/
  ...
  client.js       # браузерний auth + Socket.IO client без TS-компіляції
src/
